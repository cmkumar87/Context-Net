...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.lstm2 = nn.LSTM(input_size=Co, hidden_size=self.hidden, dropout=dropout_p, bidirectional=False)
        self.dropout = nn.Dropout(args['dropout'])
        self.fc1 = nn.Linear(self.hidden, C)
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)
        x,_ = self.lstm2(x)
        logit = self.fc1(self.dropout(x[-1]))  # dense and dropout to o/p logits
        return logit
        
cnn_lstm_args = {} 
cnn_lstm_args['embed_num'] = max_idx
cnn_lstm_args['vec'] = vec
cnn_lstm_args['class_num'] = 2
cnn_lstm_args['cuda'] = torch.cuda.is_available()
cnn_lstm_args['kernel_num'] = 100
cnn_lstm_args['kernel_sizes'] = [2]
cnn_lstm_args['hidden'] = 128 
cnn_lstm_args['embed_dim'] = args['dim']
cnn_lstm_args['dropout'] = 0.4

cnn_lstm = Model(cnn_lstm_args)
...
torch.save(cnn_lstm.state_dict(), "./best_models/cnn_lstm_best_model_weights_"+course)
torch.save(optimizer.state_dict(), "./best_models/cnn_lstm_best_model_gradients_"+course)
