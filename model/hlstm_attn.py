...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.lstm1 = nn.LSTM(input_size=D, hidden_size=self.hidden, dropout=0.2, bidirectional=False)
        self.attn = Attn(self.hidden)
        self.lstm2 = nn.LSTM(input_size=128, hidden_size=self.hidden, dropout=0.2, bidirectional=False)
    ...
    def forward(self, x):
        ...
        x,_ = self.lstm1(x) #(W,N,H)
        post_vectors = x[-1] #(N,H)
        post_vectors = post_vectors.unsqueeze(1) #(N,1,H)
        context_representations,_ = self.lstm2(post_vectors) #(N,1,H)
        ...
        attn_weights = self.attn(post_vectors[-1],context_representations)
        attended_context_representation = attn_weights.bmm(context_representations.transpose(0,1))

        #print(attended_context_representation[-1].size()) 
        #print(post_vectors[-1].size())
        #exit()
        logit = self.fc1(self.dropout(attended_context_representation[-1]))  # concatenation and dropout
        return logit
